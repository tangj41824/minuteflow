...
### DeepSeek JSON mode（新增，离线验证）

- `MINUTEFLOW_BASE_URL` / `DEEPSEEK_API_KEY` / `MINUTEFLOW_API_KEY` 配置解析已离线测试。
- 当 `MINUTEFLOW_BASE_URL` 设置时，两个 Agent 不再使用 SDK `json_schema` 结构化输出，改用 `response_format={"type":"json_object"}` 并要求模型输出 JSON；结果仍通过 Pydantic 契约与确定性二次校验。
- `parse_json_object` 覆盖普通 JSON、代码块包裹、非 JSON 与非法契约四类输入。
- 无 Key 的实时运行在发送数据前失败（exit code 2）。
...
- 没有用户 API Key，因此没有运行真实 OpenAI 或 DeepSeek 模型 smoke test，也没有产生模型费用。
- DeepSeek 的 JSON mode 解析、契约校验和配置路径已离线验证；真实 DeepSeek 模型的语义质量仍需用代表性数据评测。
